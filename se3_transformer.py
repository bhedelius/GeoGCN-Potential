...
class PointCloud:
  '''Represents a point cloud in R^3. This class calculates and stores relevant           geometric information such as the vectors, distances, directions, and
...
class WLayer(nn.Module):
  def __init__(self, k, l, c_in=1, c_out=1, d_max=1):
    ...
    self.radial = nn.Sequential(nn.Linear((1+d_max)*c_in+1, 16),
                                nn.BatchNorm1d(16),
                                nn.ReLU(),
                                nn.Linear(16, 32),
                                nn.BatchNorm1d(32),
  ...
  def prep_input(self, pc, f):
    ...
    d_max = self.d_max
    u, v = pc.graph.edges() 
    input_vector = torch.zeros(pc.graph.num_edges(), 1+c_in*(1+d_max), device=f[0].device)
    idx = 0
    for i in range(self.d_max+1):
      input_vector[:,idx:idx+c_in] = torch.sum(f[i][u]*f[i][v], dim=2)
      idx += c_in
    input_vector[:,idx] = pc.graph.edata['dist']
    return input_vector
...
class AttnBlock(nn.Module):
  def __init__(self, d_in, d_out, c_in=1, c_out=1):
    ...
    self.wk_layers = nn.ModuleList([
                      nn.ModuleList([WLayer(k, l, c_in, c_out, d_max = d_in)
